                   ...
                   bool scRna)
{
    ...
    if (!scRna) {
        ...
        sampleIds = as<vector<string>> (rownames(expressionMatrixRcpp));
        geneIds = as<vector<string>> (colnames(expressionMatrixRcpp));
    }
    ...
    gsea = new Gsea(geneSets, expressionMatrix, geneIds, sampleIds, threads, scRna);
}
