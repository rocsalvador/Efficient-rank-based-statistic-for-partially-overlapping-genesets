...
\ptitle{gseacc} The main challenge to run the Enrichment Score with scRna-seq datasets is its large size. This is why gseacc implements a function that allows to run the GSEA algorithm in chunks. This chunks are obtained by splitting the expression matrix by samples, in the case of \texttt{Anndata} objects using the \texttt{chunk\_X} function. Since the algorithm is run in different function calls and storing the previous results in the RAM is not possible, different output files are generated. The n most variable gene sets across the samples can be obtained by running the \texttt{filterResults} function from the \texttt{gseacc} package. (\ref{fig:chunkedgsea})
...
\ptitle{The dataset \label{pt:rnaseqdata}} For the RNA-seq experiment, the expression matrix from the GSE121212 experiment \cite{gse121212} was used. This dataset contains gene counts for different samples of human skin and the following phenotypes:
...
It has aproximatelly $10\cdot10^9$ counts, that is  $10^3$ orders of magnitude larger than the Rna-seq dataset \ref{pt:rnaseqdata}.

\ptitle{Steps}

The running time for the GSEA was of 9h and 46min using 8 threads, with an average of 4.9 GB of RAM usage, with the expression matrix splitted in 100 chunks.
